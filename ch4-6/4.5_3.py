"""Figure (3): two genuine hemispherical surfaces facing away from the origin.
...
# This particular reference looks along a direction in the xy-plane.
# Its projection puts +x upward, +y downward, and +z to the right.
# x and y overlap on the page but remain perpendicular axes in 3-D.
angle = np.deg2rad(18.0)
VIEW = np.array([np.sin(angle), np.cos(angle), 0.0])
CAMERA_UP = np.array([1.0, 0.0, 0.0])
...
p = pv.Plotter(off_screen=save_image is not None, window_size=(650, 800))
...
def add_hemisphere_outline(sign, radius, center):
    """Exact spherical silhouette, recomputed when the camera rotates."""
    outline = pv.PolyData()
    angles = np.linspace(0, 2*np.pi, 1000)

    def update(camera, event=None):
        normal = np.array(camera.GetViewPlaneNormal())
        normal /= np.linalg.norm(normal)
        right = np.cross(camera.GetViewUp(), normal)
        right /= np.linalg.norm(right)
        up = np.cross(normal, right)
        points = np.array([sign*center, 0, 0]) + radius * (
            np.cos(angles)[:, None]*right + np.sin(angles)[:, None]*up
        )
        keep = sign*(points[:, 0] - sign*center) <= 1e-10
        indices = np.flatnonzero(keep[:-1] & keep[1:])
        outline.points = points
        outline.lines = np.c_[np.full(len(indices), 2), indices, indices+1].ravel()

    update(p.camera)
    actor = p.add_mesh(outline, color="black", line_width=1.8,
                       lighting=False, reset_camera=False, render=False)
    front.AddActor(actor)
    p.camera.AddObserver("ModifiedEvent", update)
...
# Each surface is exactly half a sphere of radius RADIUS.
# The upper half-sphere opens upward; the lower one opens downward.
RADIUS = 1.30
CENTER = 2.05  # Distance from the origin to each sphere's center; must exceed RADIUS.
assert CENTER > RADIUS > 0
...
for sign in (1.0, -1.0):
    x = sign * (CENTER - RADIUS * np.cos(phi))
    y = RADIUS * np.sin(phi) * np.cos(theta)
    z = RADIUS * np.sin(phi) * np.sin(theta)
    # (x - sign*CENTER)^2 + y^2 + z^2 = RADIUS^2.
    add_surface(pv.StructuredGrid(x, y, z))
    add_hemisphere_outline(sign, RADIUS, CENTER)
    ring_t = np.linspace(0, 2*np.pi, 480)
    ring = np.c_[np.full_like(ring_t, sign*CENTER),
                 RADIUS*np.cos(ring_t), RADIUS*np.sin(ring_t)]
    add_curve(ring, dashed=True)

# True perpendicular axes share the origin. Their projection matches the image.
add_axis((1, 0, 0), 3.30, "x")
add_axis((0, 1, 0), 10.15, "y")
add_axis((0, 0, 1), 2.75, "z")
finish(3)
