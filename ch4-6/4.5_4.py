"""Figure (4): paraboloid z=(x^2+y^2)/2 with its vertex at the origin.
...
R_MAX = 2.0
r, t = np.meshgrid(np.linspace(0, R_MAX, 130),
                   np.linspace(0, 2*np.pi, 260), indexing="ij")
x, y, z = r*np.cos(t), r*np.sin(t), 0.5*r*r
add_surface(pv.StructuredGrid(x, y, z))
t = np.linspace(0, 2*np.pi, 480)
rim = np.c_[R_MAX*np.cos(t), R_MAX*np.sin(t), np.full_like(t, 0.5*R_MAX**2)]
add_curve(rim, dashed=True)

# All three shafts meet exactly at the paraboloid's vertex (0,0,0).
add_axis((1, 0, 0), 5.75, "x")
add_axis((0, 1, 0), 3.15, "y")
add_axis((0, 0, 1), 3.00, "z")
finish(4)
