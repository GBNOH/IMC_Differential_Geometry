"""Figure (1): ellipsoid x^2/a^2+y^2/b^2+z^2/c^2=1.
...
a, b, c = 1.35, 1.55, 1.05
theta, phi = np.meshgrid(np.linspace(0, 2*np.pi, 240),
                         np.linspace(0, np.pi, 140), indexing="ij")
mesh = pv.StructuredGrid(a*np.sin(phi)*np.cos(theta),
                         b*np.sin(phi)*np.sin(theta), c*np.cos(phi))
add_surface(mesh)

# Equator z=0 and the narrow meridian y=0, as in the reference.
t = np.linspace(0, 2*np.pi, 480)
equator = np.c_[a*np.cos(t), b*np.sin(t), np.zeros_like(t)]
meridian = np.c_[a*np.cos(t), np.zeros_like(t), c*np.sin(t)]
for curve in (equator, meridian):
    normals = curve / np.array([a*a, b*b, c*c])
    visible = normals @ VIEW >= 0.0
    add_curve(curve, keep=visible)
    add_curve(curve, keep=~visible, dashed=True)

# The opaque ellipsoid hides the interior portions of the axes naturally.
add_axis((1, 0, 0), 5.20, "x", on_top=False)
add_axis((0, 1, 0), 2.70, "y", on_top=False)
add_axis((0, 0, 1), 2.05, "z", on_top=False)
finish(1)
