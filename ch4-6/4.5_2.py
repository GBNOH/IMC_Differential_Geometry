"""Figure (2): one-sheet hyperboloid x^2+y^2=WAIST^2+K*z^2.
...
WAIST, K, Z_MAX = 0.70, 0.27, 2.0
t, z = np.meshgrid(np.linspace(0, 2*np.pi, 240),
                   np.linspace(-Z_MAX, Z_MAX, 180), indexing="ij")
radius = np.sqrt(WAIST**2 + K*z*z)
add_surface(pv.StructuredGrid(radius*np.cos(t), radius*np.sin(t), z))

t = np.linspace(0, 2*np.pi, 480)
for height in (-Z_MAX, 0.0, Z_MAX):
    radius = np.sqrt(WAIST**2 + K*height**2)
    ring = np.c_[radius*np.cos(t), radius*np.sin(t), np.full_like(t, height)]
    if height == 0.0:
        visible = ring[:, :2] @ VIEW[:2] >= 0.0
        add_curve(ring, keep=visible)
        add_curve(ring, keep=~visible, dashed=True)
    else:
        add_curve(ring, dashed=True)

add_axis((1, 0, 0), 5.25, "x")
add_axis((0, 1, 0), 2.65, "y")
add_axis((0, 0, 1), 2.85, "z")
finish(2)
